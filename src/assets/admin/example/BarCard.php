...
class BarCard extends WatchComponent
{
    ...
    {
        $echart = new LineChart('98px','180px','bar');
        $echart->hideLegend();
        $echart->xAxis(array_fill(0,count($data),''), [
            'show' => false,
        ]);
        $echart->series('趋势', $data, [
            'areaStyle' => [],
            'showSymbol' => false,
        ]);
        $echart->setOptions([
        ...
        return Card::create([
            Html::create([
                Html::create('楼盘')->style(['marginLeft' => '20px', 'color' => '#2c2c2c', 'fontSize' => '16px']),
                $this->component,
